...
public class PersonalityTypeTest {
    public static void main(String[] args) {
        ...
        String[] people = {"R","T","C","F","J","M","A","N"};
        String[] indicators = {"RT","CF","JM","AN"};

        for(String person : people){
            total.put(person,0);
        }
        for(int i=1; i<=7; i++){ // {1=3, 2=2, 3=1, 4=0, 5=1, 6=2, 7=3}
            if(i>=4){
                score.put(i,i-4);
            }
            else{
        ...
        for(int i=0; i<survey.length; i++){
            ...
            int num =0;
            num += score.get(choices[i]);
            if(choices[i]<4){
                total.replace(temp[0],num+total.get(temp[0]));
            }
            else{
                total.replace(temp[1],num+total.get(temp[1]));
            }
        }
        for(String personality : indicators){
            String[] temp = personality.split("");
            if( total.get(temp[0]) >= total.get(temp[1]) ){
